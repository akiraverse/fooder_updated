...
const DeleteMenu = ({ selectedMenu }: {selectedMenu: IMenu}) => {
	...
	const handleSubmit = async (e: FormEvent) => {
		try {
			...
			if (data?.status) {
				setIsShow(false)
				toast(data?.message, {hideProgressBar: false, containerId: `toastMenu`, type: `success`})
				setTimeout(() => router.refresh(), 100)
			} else {
				toast(data?.message, {hideProgressBar: false,containerId: `toastMenu`, type: `warning` })
			}
		} catch (error) {
			console.log(error)
			toast(`Something Wrong`, {hideProgressBar: false,containerId: `toastMenu`, type: `error` })
		}
	}
