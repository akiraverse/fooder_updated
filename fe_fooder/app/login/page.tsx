...
const LoginPage = () => {
	...
	const handleSubmit = async (e: FormEvent) => {
		try {
			...
			if (data.status == true) {
				...
				if (role == `MANAGER`) setTimeout(() =>
				router.replace(`/manager/dashboard`), 1000)
				else if (role == `CASHIER`) setTimeout(() =>
				router.replace(`cashier/dashboard`), 1000)
			}
	...
	return (
		<div className="bg-login bg-no-repeat bg-cover w-screen h-screen text-blue-950">
			...
			<div className="w-full h-full flex justify-center items-center p-5">
				<div className="w-full md:w-6/12 lg:w-4/12 min-h-[600px] border rounded-xl bg-white p-5 flex flex-col items-center relative">
					<div className="absolute bottom-0 left-0 w-full py-2 text-center">
						<small className="text-slate-600">Copright @2024</small>
					</div>
					...
					<form onSubmit={handleSubmit} className="h-auto w-auto my-2">
						<div className="flex my-4 border">
							...
							<input type="text" className="bg-white p-2 grow rounded-r-md focus:border-none" value={email}
								onChange={e => setEmail(e.target.value)} placeholder="Email" id={`email`} />
						</div>

						<div className="flex w-full my-4 border">
							<div className="bg-primary rounded-l-md p-3">
								<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4">
							...
							<input type={showPassword ? `text` : `password`} className="bg-white p-2 grow" value={password}
						...
						<div className="my-4">
							<button type="submit" className="bg-slate-800 hover:bg-primary w-full p-2 rounded-md text-white">
								login
							</button>
						</div>
					</form>
